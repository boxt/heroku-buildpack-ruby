...
class LanguagePack::Ruby < LanguagePack::Base
  ...
  def self.slug_vendor_base
    ...
    error "Problem detecting bundler vendor directory: #{@slug_vendor_base}" unless $CHILD_STATUS.success?
    slug_vendor_base
  end
  ...
  def setup_export
    instrument "ruby.setup_export" do
      ...
      set_export_override "PATH",     paths.map { |path| %r{^/.*} !~ path ? "#{build_path}/#{path}" : path }.join(":")
  ...
  def install_ruby
    instrument "ruby.install_ruby" do
      ...
      installer.fetch_unpack(ruby_version, build_ruby_path, true) if ruby_version.build?
    ...
    true
  rescue LanguagePack::Fetcher::FetchError => e
    ...
    message << "Note: Only JRuby 1.7.13 and newer are supported on Cedar-14" if ruby_version.jruby?
  ...
  def install_jvm(forced = false)
    instrument "ruby.install_jvm" do
      @jvm_installer.install(ruby_version.engine_version, forced) if ruby_version.jruby? || forced
    end
  end
  ...
  def setup_ruby_install_env
    instrument "ruby.setup_ruby_install_env" do
      ...
      ENV["JAVA_OPTS"] = default_java_opts if ruby_version.jruby?
    end
  end
  ...
  def node_preinstall_bin_path
    ...
    path        = run("which node")
    @node_preinstall_bin_path = if path && $CHILD_STATUS.success?
                                  path
                                elsif run("#{legacy_path}/node -v") && $CHILD_STATUS.success?
                                  legacy_path
                                else
                                  false
                                end
  ...
  def yarn_preinstall_bin_path
    ...
    path = run("which yarn")
    @yarn_preinstall_bin_path = if path && $CHILD_STATUS.success?
                                  path
                                else
                                  false
                                end
